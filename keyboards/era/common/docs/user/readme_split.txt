...
■ RGB 절전
   VIA CONFIGURE -> SYSTEM -> SLEEP에서 마지막 키 입력 후 RGB가 꺼질
   시간을 1/3/5/10/30/60분 중에서 고릅니다. 기본값은 10분입니다.
   EEPROM SYNC가 켜져 있으면 이 설정도 보통 양쪽에 맞춰집니다.

   DUAL-HOST에서는 각 유닛이 자기 PC와 자기 키 입력 시간을 기준으로
   독립적으로 절전합니다. HOST-PEER에서는 어느 유닛의 키를 눌러도 HOST의
   입력 시간으로 계산하며, PEER RGB도 HOST와 함께 켜지고 꺼집니다.
   Custom VIA에서는 같은 설정을 1~65535초 범위에서 1초 단위로 지정할 수
   있습니다. PC의 정상 USB Suspend나 USB frame 상실은 이 타이머를 기다리지
   않고 별도의 즉시 절전 원인으로 동작합니다.

■ 빨간색 LED 신호
   키보드가 깨어 있는 동안 RGB 설정과 관계없이 키 전체가 빨간색으로
   상태를 알립니다. 절전 중에는 상태 신호가 RGB를 강제로 깨우지 않습니다.
...
■ RGB SLEEP
   Set the delay after the last key activity in VIA CONFIGURE -> SYSTEM -> SLEEP:
   1/3/5/10/30/60 minutes. The default is 10 minutes. With EEPROM SYNC enabled,
   this stored setting normally converges on both units.

   In DUAL-HOST each unit still evaluates its own PC, stored value and local key
   activity independently. In HOST-PEER, keys on either unit refresh the HOST's
   activity clock and the PEER follows the HOST's RGB sleep state. Custom VIA
   exposes the same setting as exact seconds from 1 through 65535. Normal USB
   Suspend and USB frame loss remain separate immediate sleep reasons and do not
   wait for this timer.

■ Red LED signals
   While the keyboard is awake, whole-keyboard red status alerts appear
   regardless of RGB settings. A sleeping keyboard is not forced awake merely
   to show a status alert.
