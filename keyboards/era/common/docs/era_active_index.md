...
`keyboards/era` holds 25 boards with a `keyboard.json`, split and non-split,
...
| storage protocol edit, the custom EEPROM / ERA NVM boundary, replacement Apply, or a storage failure path | `contracts/era_host_peer_storage_contract.md`, `contracts/era_wire_contract.md`, `contracts/era_route_contract.md` | `maps/era_source_map.md` | — |
| the external H7S State-Sync handoff or dynamic-macro transcript | `contracts/era_host_peer_storage_contract.md` for the exact `GET_KEYBOARD_VALUE` `0x06` envelope, revision meanings, macro marker transcript, compatibility boundary and what H7S must not copy; this is a consumer handoff, not an H7S implementation plan | `maps/era_source_map.md` for the QMK producers and owners | `manuals/era_performance_gates.md` for the host transcript proof |
...
| the EEPROM CLEAN path, its reboot-durable prepare, its storage quarantine, or the restart it performs | the agreed-restart row above, then `contracts/era_host_peer_storage_contract.md`'s **Why An EEPROM Clean Is An Agreed Restart**: why the restart is required at all, why **both** halves erase, why a serviced clean requires bilateral physical boot-replay proof before any commit deadline, why the erase happens at boot, and where the physical-power-loss guarantee stops are canonical only there | — | `manuals/era_performance_gates.md` |
...
| a build selector, canonical variant identity, a feature enable/disable, or a new board/keymap combination | — | `manuals/era_build_options.md` first: it carries the classification rule that decides where a new option goes, every selector with its default, the immutable five-axis tuples, and the rejected arrangements with the evidence that settled them. A new firmware option is declared in `keyboards/era/era_build_options.mk`; the identity-only controls and exact variant fragments are named separately there. Every fragment that reads a QMK switch runs from a board's `post_rules.mk`, which QMK includes **after** the keymap's `rules.mk`. `maps/era_source_map.md` locates the resolver, compiled witnesses and launcher checks | `manuals/era_performance_gates.md` when the combination is offered as evidence |
...
| building or flashing, including the Windows-to-WSL local-build handoff | — | `manuals/era_build_and_flash.md`: the mandatory `era-build keyboard:keymap` automation, what WSL must provide, and how an image reaches a keyboard. Literal machine paths and installation remain the tool adapter's, per `AGENTS.md` | `manuals/era_performance_gates.md` when the build is offered as evidence |
| the audited TOMAK_TKL UF2/manifest location or the shortest ordered hardware-acceptance route | — | `manuals/era_build_and_flash.md`'s **TOMAK_TKL Hardware Acceptance Route** | `manuals/era_capture_reading.md` only if an ambiguous overlap is escalated to a wire-diagnostic capture |
