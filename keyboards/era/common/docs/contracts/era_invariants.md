...
| Applying debounce configuration retires old deadlines, not an unresolved raw/cooked difference. The first scan under the new policy MUST reconcile every row even without another raw edge; a pending key release must not remain pressed indefinitely. `matrix_need_update` is a reconciliation-due fact owned by the debounce runtime, not the sampler's raw-change receipt. BALANCED, FAST and ADVANCED all consume it, then retain the clock-free idle path | production host tests in `tests/era_matrix_debounce_runtime` | `system/era_matrix_debounce_config.c`; `system/era_matrix_debounce_runtime.c`; `system/era_rp2040_matrix_core.c` |
