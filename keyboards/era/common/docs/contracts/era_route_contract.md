...
dedicated storage request slot at a time. A pending generic request blocks the
next storage publication; the reverse gate holds `SESSION_STATUS` and matrix
outside the generic ring from successful storage publication through result
consumption or cancellation. Due facts stay armed. Admission reads the
existing Core0 cached request-pending flag (`split/era_host_peer_storage.c`,
...
construction.** Window = standing response window × backend wire scale + 5 ms
(`ERA_SPLIT_COMMUNICATION_CORE_REQUEST_HANDOFF_MARGIN_US` 5000).
`ERA_SPLIT_PEER_RESPONSE_WINDOW_MS` 20 yields 25 / 45 / 85 ms at High /
Medium / Low. Low-level maximum must stay below the 100 ms Core1-unresponsive
judgment or the build fails. Queue-residence only; the selected request's own
window starts after BEGIN
(`split/scheduler/era_split_transport_scheduler_routes.c`,
...
`SESSION_STATUS` and the standing exchange (`AUTHORITY`, `RESTART_ARM`), and
clears only on controlled reset. Why the restart is required:
...
session that admits its PEER's matrix) and carries no exclusivity term.
Matrix admission is that fact AND exclusivity. Deriving the first from the
...
| --- | --- | --- | --- |
| `ATTACH_STATUS_REVALIDATION` | `SESSION_STATUS` | bootstrap, policy, discovery, liveness, recovery | yes |
...
| core0-originated cadence in a live relation | none | **Nothing core0 originates runs on a cadence in either relation.** HOST-PEER's PEER keeps one core0-selected route (matrix source-push); the two relations run different section sets on the same envelope (`era_wire_contract.md`) |
| `ERA_SPLIT_HOST_SOURCE_RESPONSE_POLL_PERIOD_MS` | 10 | HOST-PEER standing period. **Owner decision 2026-08-09** halved it from 20 ms — no runtime route deadline in the core0 set, so shortening costs core0 no wake. Settled by capture reading, not a build (`era_capture_reading.md`) |
| `ERA_HOST_PEER_RGB_STATE_SNAPSHOT_PUBLISH_PERIOD_MS` | 10 | defined as the HOST-PEER standing period; arms on DUAL-HOST Right too — the one core0 wake still tracking a poll period |
| `ERA_SPLIT_PEER_RESPONSE_WINDOW_MS` | 20 | **not a cadence.** A non-OK exchange latches `stopped` and core1 originates nothing until core0 republishes the plan. Deadline re-armed from the poll decision, not completion |
| `ERA_SPLIT_DUAL_RUNTIME_POLL_MS` | 1 | **one period, run unconditionally** (owner decision 2026-08-01; halved from 2 ms 2026-08-09). No activity window, quiet rate, or hint. A local dirty section is due immediately. **This period is the responder's latency bound.** 1 / 2 / 4 ms High / Medium / Low = × `era_split_transaction_backend_wire_scale()` |
...
| failure streak | `ERA_HOST_PEER_STORAGE_MAX_FAILURES` | `ERA_HOST_PEER_STORAGE_PEER_SILENCE_MS` 1000 / retry; derived in `split/era_host_peer_storage.h` as 2 × `ERA_HOST_PEER_STORAGE_PEER_STALL_WORST_MS` 500 |
| probe backoff | shift-capped retry | `ERA_HOST_PEER_STORAGE_RETRY_MS` 25 << streak, streak cap `ERA_HOST_PEER_STORAGE_BACKOFF_MAX_SHIFT` 6, delay cap `ERA_HOST_PEER_STORAGE_BACKOFF_MAX_MS` 1000. Distinct from the quiet deadline; only this is a backoff. Live where `ERA_SPLIT_EEPROM_SYNC_ENABLE` compiles the engine (`era_source_map.md`) |
...
**The period is not measured and its bound is the wire, not core0.** Measured
2026-08-16: the initiator's core1 busy-polled the wire, 258 µs/exchange;
WFE-parked 160 µs, one park per byte. Sleep fractions live in
`era_performance_gates.md` **Fixed Baselines**. At 1 ms and 460800 a
worst-case exchange is ~0.80 ms with 0.2 ms idle; an overrun re-arms a past
deadline and the cadence stops rather than degrades. Every `_Static_assert` on
a poll period is an upper bound against the responder's silence limit, so a
too-short period cannot fail a build. The retired activity-gated two-rate
cadence armed a level rather than an edge and made the period unreachable; a
...
ready initiator result, or the standing state's change sequence. Result
carries lane facts; standing state carries the response section set (see
**One carrier for the response section set**). A request core1 consumed
without a result (full result ring) unsticks on the next authority sample.
...
SESSION and both push result kinds never coalesce. Device evidence 2026-08-30
DUAL-HOST macro Apply: a ~125 ms responder Core0 flash window accumulated
`crsp full=32/noack=45` while the initiator's pending fall sat behind
`SESSION_STATUS` for 115 ms.
...
At 1 ms and 460800 the worst case is ~0.80 ms with 0.2 ms idle.
...
| Plan field | Rule |
| --- | --- |
| owner epoch, relation generation, enable bit, period, eligible push mask, latest-state section bodies | published **on change, never on a pass or a timer** |
...
| publication route-due bit | DUAL-HOST only. `era_split_transport_scheduler_scan_idle()` in `split/era_split_transport_scheduler.c` reads it. Consumed at the top of `era_split_transport_scheduler_publish_standing_plan()` in that file, before the plan is built. `era_split_transport_scheduler_refresh_route_due_flags()` in `scheduler/era_split_transport_scheduler_timing.c` is the backstop, not the consumer. Without a publish-side consumer a DUAL-HOST Left held `scan_idle()` false per scan until housekeeping |
...
ACK and carries no section. Eligibility is untouched: it narrows *when* a
section crosses, never *whether* the relation may carry it.
...
Selection and enqueue run only from the cold core0 task. Scan hooks consume
only the cached storage-active suppression token. The responder never selects
a storage route.

| Initiator may select only when all hold |
| --- |
| confirmed relation that admits it as initiator |
| local requested policy enabled |
| both endpoints advertise bulk-page support |
| no mandatory status revalidation due |
| no CLEAN quarantine on this half |
| dedicated core1 result capacity available |
...
not. **There is one storage lane, not one per relation.** The mask admits
only `SESSION_STATUS` while exclusive.
...
`split/era_split_transport_scheduler.c`'s backend teardown-and-rebuild
window. The scheduler asserts the dwell outlasts two backed-off probes with
their slowest response windows. A silence floor recorded a working level as
failed; the refusal lives in `split/era_split_link.h` **Reconciliation**.
...
in `split/era_split_transport_scheduler.c`;
...
Core0 selects three things, in this order:

1. Pending `SESSION_STATUS` bootstrap, policy-generation, or recovery
   revalidation.
2. Active HOST-PEER storage step.
...
(INPUT, AUTHORITY, RGB, time anchor, ACTIVITY, visual pressed-baseline,
storage-pending). Consequences: poll unconditional; ACTIVITY's window
derivation keeps a fresh-defaults image off the wire; Right is the time
authority. Owner record 2026-08-09: RGB SYNC targets complete
synchronization, behaving as HOST-PEER. Left marks the publication
route-due bit for INPUT edge, activity change, and visual baseline.
Right's changes ride snapshot rebuild.
...
- **`ERA_SPLIT_RESPONDER_SILENCE_MS` 100 is canonical here.** Both poll
  ...
  `era_split_transport_scheduler_relation_stale_ms()`). Every "100 ms
...
| in-flight expiry | pending initiator request neither consumed nor expired | beside the cancel gate | `split/era_split_transport_scheduler.c` |

Initiator-watch arms only when: initiator role, live `CORE1` lease, known
peer, no stop report, and a **granted** standing plan
(`relation_generation != 0`). Ungated, the watch fires every bound on a
healthy powered-without-USB pair.

Bound: `ERA_SPLIT_CORE1_UNRESPONSIVE_MS` == `ERA_SPLIT_RESPONDER_SILENCE_MS`
100, never the `ERA_SPLIT_SESSION_STALE_MS` 0 override. `_Static_assert`
holds 2 × `ERA_SPLIT_STANDING_LIVENESS_MS` 50 inside it, at equality.
...
`timer_read32()` costs about 0.42 µs (`platforms/chibios/timer.c`,
...
**There is one clock.** `CH_CFG_ST_FREQUENCY` is 1,000,000;
`st_lld_get_counter()` returns `TIMER->TIMERAWL` — the register
`timer_read32()` (`platforms/chibios/timer.c`) reads. Three alternatives
each buy 0.021 µs.
...
- Producers set latches OR-only, under `ATOMIC_BLOCK_RESTORESTATE`. Never
  clear; never call scheduler planning.
