...
frame loss). Split local arm:
`era_split_keyboard_local_usb_sleep_state()` in `split/era_split_keyboard.c`
ORs configure-state `SUSPEND` with `era_usb_session_frames_lost()` in
`system/era_usb_session.c` (`ERA_USB_SESSION_SOF_STALE_MS` 300; false until a
host has configured this board; false under the reattach hold). Model:
`era_board_adoption.md` **Non-Split Board Baseline**. Ownership:
**Lighting Sleep Ownership**. Suspend grace and host-open close govern
authority only.
...
Override, not a second owner: it re-asserts while it runs; the resolver writes
only on an edge of its own value. **A HOST-PEER HOST publishes the resolved
decision and never the raw gate.** DUAL-HOST capture zeroes the sleep bit;
apply publishes and does not write the gate (`era_wire_contract.md`).
