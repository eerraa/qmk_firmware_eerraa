...
Read when: 키보드 리포트 폭, `KEYBOARD_EPSIZE`, NKRO 토글, boot 프로토콜 처리,
KKUK 펄스 가운데 하나를 건드리기 전 — 또는 "동시입력이 몇 키까지 되는가"가
판단의 대상이 될 때
...
항목으로 소개하는 것(`user/readme.txt`의 Anti-Ghosting 및 NKRO 항목,
...
Anti-Ghosting 켜짐으로 두 키 이상을 길게 누르고 있을 때 호스트가 키를 잃지
않는지.
