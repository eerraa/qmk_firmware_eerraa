...
| `era_split_rgb_sleep_policy` | with the RGB Sleep master on, local sleep is the OR of explicit suspend, frame loss, and the optional matrix-idle timeout; master off suppresses all three reasons, while zero timeout suppresses only idle; stock preset validation is exactly 1/3/5/10/30/60 minutes; exact seconds project down on GET without modifying storage. Tests `split/era_split_rgb_sleep_policy.h` |
