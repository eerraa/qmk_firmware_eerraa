...
| `quantum/process_keycode/process_tap_dance.[ch]`, `quantum/quantum.c` | Weak `tap_dance_remap_keycode()` / `tap_dance_get_tapping_term()` in `quantum/process_keycode/process_tap_dance.c`; `process_record_quantum()` in `quantum/quantum.c` applies the remap both before preprocessing and after a layer-changing relookup. | Weak upstream-defaulted hooks; ERA feature supplies the override. |
