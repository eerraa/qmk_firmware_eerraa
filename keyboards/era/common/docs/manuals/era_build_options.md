...
| `ERA_STORAGE_QUIET_DEFER_MS` | `500` | Rule 2's stated exception. Not because no ERA file reads it — `sirind/common/tomak_common.c` uses its value, and it is the only one (`git grep -l ERA_STORAGE_QUIET_DEFER_MS -- 'keyboards/era/**/*.c' 'keyboards/era/**/*.h'`) — but because the readers that need the **default**, `quantum/eeconfig.h` and `quantum/rgb_matrix/rgb_matrix.c`, are preprocessed before any ERA header, so no ERA `#ifndef` can reach them. This cell said *three* board files until 2026-08-17, which it was before the tomak family's three board units became one shared one: a count of files survived the change that made them one file |
