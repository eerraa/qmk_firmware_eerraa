...
| `era_host_peer_storage.[ch]` | seven-domain manifest, latest-state capture, CRC/revision, quiet/due facts, the initiator/responder state machines for pull and push, latest-change-wins arbitration and its cell queues, and the guarded durable Apply on both roles. Apply owns the staged-old public EEPROM read facade, the raw writer/verifier/reload bypass, the checked slice cursor, rollback and repair-required episode states, the single publication flip and the post-flip immutable publication order. The unit also owns recovery, diagnostics, the persisted recency layer (baseline record and divergence counters), the cached request-pending route-admission fact, and the three relation-admission helpers, which are the engine's whole coupling to the relation mode and the only place a new serviced relation is added |
...
| `storage/era_storage_slice_swap.h` | header-only 32-byte staged-old slice cursor: old-view composition, checked write/verify progression, bounded reverse rollback, retained-prefix absorption for foreign local writes, publication flip and repair-required retry state |
...
- `quantum/dynamic_keymap.c` owns dynamic keymap and macro NVM images;
  `quantum/nvm/eeprom/nvm_dynamic_keymap.c` turns the macro image's existing
  final-byte valid marker into ERA wear-leveling's exact transaction boundary:
  a nonzero marker opens, payload stages cache-only, an early zero is forced
  back to nonzero, and the trailing zero after payload consolidates and closes
  only on success. Unbracketed payload/zero writes are ignored; macro reset is
  a separate durable command.
...
| `tests/era_storage_slice_swap/` | host proof of old-view publication across every slice, internal raw candidate progress, partial failure and abort rollback, retryable repair, foreign local mutation absorption, successful single flip/revision, and inactive/cross-boundary read-facade behavior |
