...
| `keyboards/era/sirind/common/tomak_common.[ch]` | tomak family board content for `tomak`, `tomak79h`, and `tomak79s`. The header preamble is the family boundary. None of the three boards keeps a `.c` |
...
| `split/era_split_board.[ch]` | split class skeleton over `split/era_split_keyboard.c`. Does not own the init trio (`matrix_init_kb` / `eeconfig_init_kb` / `via_init_kb`) |
...
| `split/era_split_keyboard.[ch]` | ERA split facade: pre/post init, task, feature reload, USB device-state change, lighting-sleep render gate, process-record, VIA entry, and the agreed-restart act table / dispatch (`split/era_split_restart_agreement.h` declares both). A split board runs QMK's wake path and none of its sleep path (`NO_USB_STARTUP_CHECK`) |
...
| `system/era_usb_session.[ch]` | local USB session facts as ERA reads them, including the frame-loss sleep detector and, non-split only, the suspend apply pair |
| `system/era_via_system.[ch]` | ERA system VIA router and task: EEPROM CLEAN confirm mask, the one restart-quiet policy, `era_via_system_restart_quiet_ok()`, and `era_via_system_eeprom_invalidate()`. Split clean hand-off: `split/era_split_keyboard.c` |
...
| `tests/era_via_exact_ms/` | host proof of exact-ms tapping/tapdance round-trip and State Sync `0x06` envelope mapping |
