...
4. **Lighting sleeps** through a separate decision. Every ERA board has the suspend state and SOF frame-loss detectors because a dead host behind a live cable can stop sending frames without announcing a suspend. TOMAK split adds its persisted input-idle timeout; DUAL-HOST evaluates each half locally, while a HOST-PEER PEER follows the HOST's resolved sleep bit.
