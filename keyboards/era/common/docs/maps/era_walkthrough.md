...
4. **Lighting sleeps** through a separate decision. Every RGB-capable ERA board compiles the QMK sleep state and has one persisted RGB Sleep master. With it on, suspend and SOF frame-loss can sleep RGB; TOMAK split adds its persisted input-idle timeout as a third reason. With it off, none of the three reasons sleeps RGB. DUAL-HOST evaluates each half locally, while a HOST-PEER PEER follows the HOST's resolved sleep bit.
