...
While `USB_INTE_DEV_SOF` is armed for remote wake, ChibiOS owns the SOFRD read.
`era_usb_session.c` rate-limits the ownership observation separately from a
successful SOFRD read; a pending DEV_SOF proves arrival, while 300 ms of
continuous ISR ownership with no pending SOF is itself the frame-loss verdict.
