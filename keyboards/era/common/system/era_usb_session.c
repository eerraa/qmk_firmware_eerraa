...
/* The once-per-millisecond observation cadence is separate from a successful
   SOFRD read. While remote wake owns DEV_SOF we deliberately do not read
   SOFRD, so using last_read_us as the cadence stamp turned that safe stand-off
   into a scan-rate USB->INTE poll. The ownership stamp also lets frames_lost()
   distinguish that deliberate observation gap from an arbitrary Core0 stall. */
static uint32_t era_usb_session_sof_last_observe_us;
static uint32_t era_usb_session_sof_isr_owner_started_us;
static uint16_t era_usb_session_sof_count;
static bool     era_usb_session_sof_seen;
static bool     era_usb_session_host_seen;
static bool     era_usb_session_sof_isr_owned;
...
bool era_usb_session_sample_frame_age(uint32_t *age_ms) {
    ...
    if (era_usb_session_sof_last_observe_us == 0 || (uint32_t)(now - era_usb_session_sof_last_observe_us) >= 1000) {
        era_usb_session_sof_last_observe_us = now;
        /* One reason remains to take no reading this millisecond, and it is
           ...
           the ISR owns this register. Track that ownership separately from an
           ordinary observation gap: a dead host can leave the waiter armed
           indefinitely, and that deliberate stand-off is still evidence once
           no DEV_SOF has arrived for the full stale window. */
        if ((USB->INTE & USB_INTE_DEV_SOF) != 0) {
            if (!era_usb_session_sof_isr_owned) {
                era_usb_session_sof_isr_owned            = true;
                era_usb_session_sof_isr_owner_started_us = now;
            }
        } else {
...
bool era_usb_session_frames_lost(void) {
    ...
    if (era_usb_session_sof_isr_owned) {
        /* SOFRD belongs to the ChibiOS ISR while DEV_SOF is enabled for remote
           wake, so do not "fix" this by reading the side-effect register here.
           A pending DEV_SOF proves a frame has arrived and the ISR merely has
           not consumed it yet. With no pending frame, however, an ownership
           interval that itself spans the full loss threshold is observed
           absence, not the flash-starvation case below. This closes the case
           where an alternative wake/control event leaves DEV_SOF enabled and
           the host then disappears: the old stale-last-read guard returned
           false forever even though no frame arrived. */
        if (now == 0 || era_usb_session_sof_isr_owner_started_us == 0 ||
            (USB->INTS & USB_INTS_DEV_SOF) != 0) {
            return false;
        }
        return era_usb_session_policy_isr_owned_frames_lost(
            (uint32_t)(now - era_usb_session_sof_isr_owner_started_us),
            (uint32_t)(now - era_usb_session_sof_last_change_us),
            false,
            ERA_USB_SESSION_SOF_STALE_US);
    }
