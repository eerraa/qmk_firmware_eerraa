...
era_nvm_result_t era_nvm_replace(era_nvm_t *nvm, uint32_t address, const void *data, size_t length, era_nvm_origin_t origin) {
     ...
     * setting with no retry. The NVM boundary therefore refuses only writes
     * that overlap the staged macro domain; unrelated durable writes must
     * remain admissible while an upload is open. */
