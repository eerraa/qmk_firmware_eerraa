...
static bool era_split_keyboard_local_usb_sleep_state(uint8_t configure_state) {
    // Two detectors, one decision (ERA policy, owner decision 2026-08-11): the
    // host's explicit suspend, and the loss of USB frames. Gating this on
    ...
    return configure_state == USB_DEVICE_STATE_SUSPEND || era_usb_session_frames_lost();
}
...
static bool era_split_keyboard_resolve_lighting_sleep(void) {
    ...
    if (owner_is_wire) {
        ...
        sleep = era_split_keyboard_local_usb_sleep_state(era_usb_session_configure_state());
    }
