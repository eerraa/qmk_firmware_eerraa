 ...
 * branch, in every effect, whether or not a blink is running -- and that is
 ...
 *    of the thousands of passes a blink spans. The read is cached to a
 *    millisecond and still is not free, and while someone is typing the blink
 ...
 * one flag; the next pass sees the flag and writes the PWM back. No clock is
 * read on the scan path at all, and the cost of an effect that is off is the
 * same as the cost of one that is running.
 *
 * `chVTSet()` resets an armed timer before re-arming it (`chVTSetI` calls
 * `chVTResetI` first), so a key pressed during a blink simply restarts the
 * interval, which is the behaviour a reader expects and costs no bookkeeping.
 *
...
enum {
    /* Blink speed is 1..10 with 10 the fastest, which is the range the shipped
       ...
       a range and not a duration: 20 ms a step puts the slowest blink at
       ...
       notice as lag. */
    ERA_BACKLIGHT_BLINK_UNIT_MS = 20,
    ...
    /* Any byte but zero would do; a fresh EEPROM reads zero and must land on
       the defaults rather than on effect NONE with a zero period. */
...
static virtual_timer_t backlight_blink_vt;
...
static void era_backlight_apply_defaults(era_backlight_config_t *config) {
    ...
    config->effect           = ERA_BACKLIGHT_EFFECT_NONE;
    config->breathing_period = ERA_BACKLIGHT_DEFAULT_PERIOD;
    config->blink_speed      = ERA_BACKLIGHT_DEFAULT_SPEED;
}
...
static bool era_backlight_normalize(era_backlight_config_t *config) {
    ...
    config->blink_speed      = era_backlight_clamp(config->blink_speed, ERA_BACKLIGHT_SPEED_MIN, ERA_BACKLIGHT_SPEED_MAX);

    return memcmp(&before, config, sizeof(before)) != 0;
}

/* The level this effect shows between keypresses. Blink-in is dark until a key
   is pressed, which is the whole of what distinguishes it from blink-out. */
static uint8_t era_backlight_steady_level(void) {
    return backlight_config_era.effect == ERA_BACKLIGHT_EFFECT_BLINK_IN ? 0 : get_backlight_level();
}

static uint8_t era_backlight_blink_level(void) {
    return backlight_config_era.effect == ERA_BACKLIGHT_EFFECT_BLINK_IN ? get_backlight_level() : 0;
}
...
static void era_backlight_apply(void) {
    ...
    if (is_breathing()) {
        /* Also restores the level, which the steady write below then confirms
           for the blink-in case where the two disagree. */
        breathing_disable();
    }
#endif
    backlight_set(era_backlight_steady_level());
}

/* Interrupt context. One store, and deliberately nothing else: the restore is a
   ...
   layer. */
static void era_backlight_blink_expired(virtual_timer_t *vtp, void *p) {
    (void)vtp;
    (void)p;
...
void era_backlight_init(void) {
    chVTObjectInit(&backlight_blink_vt);
...
bool era_backlight_process_record(uint16_t keycode, keyrecord_t *record) {
    ...
    /* The first test is the one every key event on a board with a steady or
       breathing backlight pays, so it is a byte compare against a constant. */
    if (backlight_config_era.effect < ERA_BACKLIGHT_EFFECT_BLINK_OUT) {
        return true;
    }
    ...
    backlight_set(era_backlight_blink_level());
    /* Re-arming an armed timer is defined: `chVTSetI()` resets first. A key
       pressed mid-blink therefore restarts the interval instead of leaving the
       first deadline to cut the second blink short. */
    chVTSet(&backlight_blink_vt, TIME_MS2I((uint16_t)(ERA_BACKLIGHT_SPEED_MAX + 1 - backlight_config_era.blink_speed) * ERA_BACKLIGHT_BLINK_UNIT_MS), era_backlight_blink_expired, NULL);
    return true;
}
...
void era_backlight_set_effect(uint8_t effect) {
    ...
    /* Leaving a blink effect with a blink in flight would strand the timer and
       let its callback re-apply the old steady level one interval later. */
...
uint8_t era_backlight_get_blink_speed(void) {
    return backlight_config_era.blink_speed;
}
