...
ERA_RGBLIGHT_PULSE_ENABLE = yes
ERA_VIA_BOOTLOADER_ENABLE = yes
ERA_EEPROM_CLEAN_ENABLE = yes
ERA_STORAGE_QUIET_DEFER_MS = 500
VELOCIKEY_ENABLE = yes

# This board's content, shared with its twin: the two board files were
# string-identical after the board-token substitution, so the family owns one
# copy. Stated here rather than in a fragment of its own -- one SRC line is not
# a build option and has nothing to declare.
SRC += keyboards/era/newone/common/odessey_common.c
