...
- **개발 히스토리는 개발 브랜치에서만 답한다.** `git log -S`는 거기서 은퇴한
  이름을 찾지만, 최종 관심사만 남긴 출하 `main`에서는 큰 import 커밋에 닿거나
  아무것도 찾지 못한다:
...
이 문서의 독자가 들고 있는 것은 원본 QMK fork point 위에 최종 관심사만
재구성한 출하 `main`이고, 거기에서 그 `git show`는 펌웨어 전체를 담은 import
커밋에 닿을 수 있으므로 개발 판단의 답이 아니기 때문이다 — 그래서 결과 문장이
이제 논증의 전부다. **개발 브랜치의 세션은 `git show`를 돌려 답을 얻는다.
출하 `main`에서는 안 된다는 것이 바로 그 줄을 빼야 하는 이유다.** 독자가
믿어야 하는 것으로 쓰고, 다른 곳에 둔 논증의 라벨로 쓰지 마라 — 그렇게 쓸 수
없으면 그 거절은
...
위임해서는 안 된다.** 출하 `main`은 원본 QMK fork point 위에 최종 관심사만
재구성한 짧은 히스토리이고, 거기에서 `git log -S`는 개발 chronology의 답이
없다. 작업 브랜치가 무엇에 답하든 상관없다. 그 구분과 그것이 섬기는 규칙은
`era_active_index.md`'s **What This Repository Does Not Carry**에 있다.
