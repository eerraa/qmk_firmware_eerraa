...
token is a promotion candidate, not a deletion candidate. The commit gate runs
the same script with no argument automatically, which is the locatability
check. After doc changes: `graphify update .` from the repository root,
